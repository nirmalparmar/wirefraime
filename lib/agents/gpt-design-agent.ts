 ...
 */
const PLANNING_MODEL = process.env.DESIGN_PLANNING_MODEL || "openai/gpt-5.5";
const STREAMING_MODEL = process.env.DESIGN_STREAM_MODEL || "openai/gpt-5.5";
const CRITIC_MODEL = process.env.DESIGN_CRITIC_MODEL || "openai/gpt-5.5";
...
async function collectAgent(
  ...
  // Agent instances (OpenRouter path) use .chat() directly
  if (agent instanceof Agent) {
    const text = typeof prompt === "string" ? prompt : (prompt.parts?.[0] as any)?.text ?? "";
    const result = await agent.chat(text);
    return result.text;
  }
  // LlmAgent: use the ADK runner path
  let result = "";
...
Output ONLY the HTML starting with <!DOCTYPE html>. No markdown fences.`;
  ...
  // Deterministic guarantee: swap whatever nav the model produced for the
  // canonical shared shell (active item toggled). Runs after critique so the
  // result is consistent regardless of any regeneration above.
  finalHtml = enforceSharedShell(finalHtml, navHtml, navStyle, screenName);
...
CRITICAL: Every search string MUST include the element's unique \`data-wf-id="..."\` attribute to guarantee a match. Prefer class-attribute swaps over full element rewrites.`;
